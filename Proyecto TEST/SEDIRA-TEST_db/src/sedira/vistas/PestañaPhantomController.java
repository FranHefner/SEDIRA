...
public class PestañaPhantomController implements Initializable {
    ...
    /**
     * Método que se activa al seleccionar un phantom. Este llenara las tablas
    ...
    public void seleccionarPhantom() {
        if (ListaPhantom.size() == 0) {
            ...
            alert.setHeaderText("No existen datos para continuar el proceso de cálculo ");
            alert.setContentText("No se encuentran Phantoms válidos para continuar el cálculo. "
                    + "Por favor agregar o modificar los phantoms desde la herramienta de administración de phantoms ");
            alert.showAndWait();
        }
        choicePhantom.getSelectionModel().selectedIndexProperty().addListener(new ChangeListener<Number>() {
